...
uint8_t pinmap(int i,int j){
 return (i*3)+j;
}

uint16_t angleToPulse(int ang)                             //gets angle in degree and returns the pulse width
{  
     uint16_t pulse = map(ang,0, 180, SERVOMIN,SERVOMAX);  // map angle of 0 to 180 to Servo min and Servo max 
     return pulse;
}
...
void loop()
{
  // for (int i = 0; i < 4; i++){
  //   for (int j = 0; j < 3; j++){
  //     board1.setPWM(pinmap(i, j),0, angleToPulse(90));
  //     delay(20);
  //   }
  // }
  if(!is_stand())
...
void polar_to_servo(int leg, float alpha, float beta, float gamma)
{
  ...
  setLegPWM(leg, 0, alpha);
  setLegPWM(leg, 1, beta);
  setLegPWM(leg, 2, gamma);
}

void setLegPWM(int leg, int j, float angle){
  uint16_t pulse = angleToPulse(angle);
  Serial.print(j);Serial.print(": ");Serial.print(angle);Serial.print("pulse: ");Serial.println(pulse);
  if(board1.setPWM(pinmap(leg, j),0, pulse) == false){
    Serial.println("Error moving servo");
  }
}
